    ...
    <div class="flex justify-between items-center h-16">
      <template v-for="link in $t('header.links')">
        <nuxt-link
          ...
          :to="localePath({ name: link.routeName, params: { section: link.slug } })"
        >
          <component
            :is="link.slug + '-icon'"
            class="inline-block h-5 fill-current mb-1"
            :class="{'text-nuxt-lightgreen': $route.params.section === link.slug}"
          />
          <span
        ...
        <a v-else-if="link.type === 'external'" :key="link.href" :href="link.href">{{ link.name }}</a>
      </template>
    </div>
  </nav>
</template>
