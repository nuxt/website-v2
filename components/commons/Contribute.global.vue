<template>
  <div class="pt-4">
    ...
    <div class="pt-3 mt-6 border-t border-light-border dark:border-dark-border">
      <h3 v-if="contributors.length" class="my-2 text-light-onSurfacePrimary dark:text-dark-onSurfacePrimary">Contributors</h3>
      <div v-if="contributors.length">
        <a v-for="contributor of contributors" :key="contributor.author" :href="`https://github.com/${contributor.author}`" rel="noopener" target="_blank" class="text-light-onSurfacePrimary dark:text-dark-onSurfacePrimary rounded overflow-hidden rounded-md inline-flex mb-2 mr-2 border border-light-border dark:border-dark-border">
      ...
      <p v-if="docLink" class="text-light-onSurfaceSecondary dark:text-dark-onSurfaceSecondary pt-1 mb-8">{{ $store.state.lang.guide.contribute }} <a :href="docLink" target="_blank" rel="noopener" class="text-primary-base hover:underline">{{ $store.state.lang.guide.edit_on_github }}</a></p>
