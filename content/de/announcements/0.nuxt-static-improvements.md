...
description: Mit Nuxt Version 2.13 wurde der full-static Modus eingeführt. Außerdem wurde ein neuer Befehl nuxt export hinzugefügt, um Ihre Seiten zu pre-rendern, ohne einen Webpack-Build auszulösen, mit dem Ziel, den Rendering- und Build-Prozess zu trennen. Das einzige Problem war, dass die meisten Nuxt-Benutzer nicht in der Lage waren, das volle Potenzial der Trennung auszuschöpfen... bis jetzt.
...
- `Cache` ist nur, wenn sich der Inhalt geändert hat (Webpack-Build übersprungen)

<bar-chart-cache-build></bar-chart-cache-build>

::alert{type="next"}
Die Static Generation unserer Projekte bei Inhaltsänderungen ist jetzt **~3,6x** schneller 🚀.
::
...
Mit Nuxt v2.13 haben wir `nuxt export` und `nuxt serve` eingeführt, die speziell für Static Targets entwickelt wurden. Mit Nuxt v2.14 sind sie veraltet, da `nuxt generate` und `nuxt start` das Ziel intelligent erkennen und builden.
...
- `nuxt start` = - `nuxt start` = das Verzeichnis `dist/` zu benutzen, wie es Ihr Static Hosting tun würde (Netlify, Vercel, Surge, usw.), ideal für Tests vor dem Deployment
