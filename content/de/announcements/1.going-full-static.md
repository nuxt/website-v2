...
`nuxt generate` ist größtenteils Pre-Rendering, wenn Sie clientseitig navigieren, werden "asyncData" und "fetch" aufgerufen, _die eine Anfrage an Ihre API senden_. Viele Entwickler haben darum gebeten, einen "vollständig statischen" Modus zu unterstützen, was bedeutet, dass diese 2 Hooks bei der Navigation nicht aufgerufen werden, da die nächste Seite bereits vorgerendert wurde.
...
- `static`
  ...
  - `nuxt generate`: Bundle Ihre Nuxt-Anwendung bei Bedarf für die Production (static) und exportieren Sie Ihre Anwendung als Static HTML in das Verzeichnis `dist/`.
