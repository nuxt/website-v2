...
description: 'Lang erwartete Funktionen für JAMstack-Fans wurden in v2.13 eingeführt: vollständiger Static Export, verbessertes Smart Prefetching, integrierter Crawler, schnelleres Re-Deployment, eingebauter Webserver und neue Zieloption für die Konfiguration ⚡️'
...
\_Tipp: Sie können `nuxt start` ausführen, um einen lokalen Server zu starten, der Ihre generierte Static Anwendung bedient.
...
Nuxt hatte die Static Generierungsfunktion `nuxt generate` seit [v0.3.2](https://github.com/nuxt/nuxt.js/releases/tag/v0.3.2) (November 2016), seither haben wir sie auf verschiedene Weise verbessert, aber nie eine vollständige Static Generierung erreicht. Heute freue ich mich, ankündigen zu können, dass der vollständige Static Export nun mit Nuxt 2.13 möglich ist.

## Aktuelle Probleme

`nuxt generate` ist größtenteils Pre-Rendering, wenn Sie clientseitig navigieren, werden "asyncData" und "fetch" aufgerufen, _die eine Anfrage an Ihre API senden_. Viele Benutzer haben darum gebeten, einen "vollständigen Static" Modus zu unterstützen, was bedeutet, dass diese 2 Hooks bei der Navigation nicht aufgerufen werden, da die nächste Seite bereits vorgerendert wurde.
...
- `process.static` ist nur dann `true`, wenn `nuxt generate` ausgeführt wird, was die Entwicklung von Nuxt-Modulen oder Plugins für die Static Generierung verlangsamt
...
Beachten Sie, dass es möglich war, vollen Static Support mit dem [nuxt-payload-extractor](https://github.com/DreaMinder/nuxt-payload-extractor) Modul zu haben, aber es war ausführlicher zu benutzen und hatte Einschränkungen.

## Neue Config Option: `target`

Um die Benutzerfreundlichkeit zu verbessern und Nuxt mitzuteilen, dass Sie Ihre Anwendung auf ein Static Hosting exportieren möchten, führen wir die Option `target` in `nuxt.config.js` ein:
...
Vollständiger Static Support funktioniert nicht mit `ssr: 'false'` (was dasselbe ist wie die veraltete Option `mode: 'spa'`), da dies nur für clientseitiges Rendering verwendet wird (Single Page Applications).
...
### Wahnsinnig schnelle Static Anwendungen
...
Wir haben auch das [smart prefetching](/announcements/introducing-smart-prefetching) verbessert, das bei vollständigem Static Support auch die Payloads abruft, so dass die Navigation sofort erfolgt. 👀
...
Sobald Sie Ihre Static Nuxt-Anwendung in `dist/` generiert haben, verwenden Sie `nuxt start`, um einen Production-HTTP-Server zu starten und Ihre Static Anwendung zu deployen, die [SPA Fallback](/docs/concepts/static-site-generation#spa-fallback) unterstützt.

Dieser Befehl eignet sich perfekt, um Ihre Static Anwendung lokal zu testen, bevor Sie sie an Ihren bevorzugten Hosting-Anbieter weitergeben.
...
- `server`
  ...
  - `nuxt build`: Builded deine Nuxt Anwendung für production
  - `nuxt start`: Startet Production Nuxt-Anwendung
- `static`
  - `nuxt dev`: Startet den Development Server (static)
  - `nuxt generate`: Builded Ihre Nuxt-Anwendung bei Bedarf für die Production (static) und exportieren Sie Ihre Anwendung als Static HTML in das Verzeichnis `dist/`.
