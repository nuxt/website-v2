...
- You cannot test the [SPA fallback](/docs/2.x/x/concepts/static-site-generation#spa-fallback) in development, the fallback is a client-only version of your Nuxt application that loads when hitting a 404 page
...
- Fallback to client-side rendering on 404, errors and redirects (see [SPA fallback](/docs/2.x/x/concepts/static-site-generation#spa-fallback))
...
If you want to exclude a bunch of routes, use the [generate.exclude](/docs/2.x/x/configuration-glossary/configuration-generate#exclude). You can keep using [generate.route(/docs/2.x/2.x/configuration-glossary/configuration-generate#routes) to add extra routes that the crawler could not detect.
...
Once you statically generated your Nuxt app into `dist/`, use `nuxt start` to start a production HTTP server and serve your static app, supporting [SPA Fallback](/docs/2.x/x/concepts/static-site-generation#spa-fallback).
