...
You might need to customize the host and port, when deploying to platforms like Heroku, Azure etc.. you can update the start command to set these:
::code-group
```bash [Windows]
"start": "env NITRO_HOST=127.0.0.1 env NITRO_PORT=3001 .output/server/index.mjs"

// more specifically for Heroku
"start": "env NITRO_HOST=0.0.0.0 env NITRO_PORT=$PORT .output/server/index.mjs"
```
```bash [Linux/Mac]
"start": "NITRO_HOST=127.0.0.1 NITRO_PORT=3001 node .output/server/index.mjs"
```
::
