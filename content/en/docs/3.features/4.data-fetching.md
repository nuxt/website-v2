...
`fetchKey`: `String` or `Function` (defaults to the component scope ID or component name), a key (or a function that produces a unique key) that identifies the result of this component's fetch (available on Nuxt 2.15+). It is used to map SSR-data to client-data in the hydration process, where SSR-data is the component's data after calling `fetch()` during server-side rendering, and client-data is the component's data at client side. [More information available in original PR](https://github.com/nuxt/nuxt.js/pull/8466).
