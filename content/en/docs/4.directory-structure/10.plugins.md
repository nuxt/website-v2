...
::alert{type="warning"}
Don't use `Vue.use()`, `Vue.component()`, and globally, don't plug anything in Vue **inside** the `inject` function, dedicated to Nuxt injection. It will cause memory leak on server-side.
::
