...
This allows you to create a client api - server api pattern using nuxt alone. This means that you can use your client-side (e.g., a component) to call the api routes in your server middleware, i.e., your server-side middleware.

One benefit of this pattern is that the server middleware exists on the server (like most middleware), not on the client. This means that you can handle environment variables and secrets in the server-side middleware, without exposing that information to the user.
