...
You can access the Nuxt [context](/docs/2.x/x/concepts/context-helpers) within the fetch hook using `this.$nuxt.context`.
...
`asyncData` is only available for [pages](/docs/2.x/x/directory-structure/pages) and you don't have access to `this` inside the hook.
...
Learn more about the [watchQuery property](/docs/2.x/x/components-glossary/pages-watchquery) and see the list of available [keys in contex(/docs/2.x/2.x/concepts/context-helpers).
