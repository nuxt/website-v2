...
Vuex Store instance. **Available only if the [vuex store](/docs/2.x/x/directory-structure/store) is set**.
...
Environment variables set in `nuxt.config.js`, see [env api](/docs/2.x/x/configuration-glossary/configuration-env).
...
It's not possible to use `redirect` or `error` in [client-side Nuxt plugin](/docs/2.x/x/directory-structure/plugins#client-or-server-side-only) due to hydration errors (client content would be different from what it'd expect from the server).
