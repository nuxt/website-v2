...
`store` ディレクトリの中にあるすべての  `.js`  ファイルは[名前空間付きモジュール](http://vuex.vuejs.org/ja/modules.html)に変換されます  (`index`  はルートモジュールになる) 。 `state`  の値は不必要に状態がサーバサイドで共有されてしまうことを避けるため、常に `function` であるべきです。
...
このアクションは受け取るのはプライマリモジュール (`store/index.js` 内の) だけです。ここから他のモジュールのアクションに呼び出しを繋いでいく必要があります。
