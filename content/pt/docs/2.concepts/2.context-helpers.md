...
Primeiramente, o contexto é usado para dar acesso à outras partes da aplicação do Nuxt, por exemplo, a memória do Vuex ou a instância subjacente de `connect`. Assim, temos os objetos `req` e `res` dentro do contexto disponível no lado do servidor e o `store` sempre disponível. Agora, temos acesso ao HMR (Módulo de Recarregamento / Substituição Instantanea)
...
Além dos atalhos dentro do contexto, existêm também outros pequenos auxiliares presentes dentro da sua aplicação do Nuxt.
...
Quando você quiser atualizar a página atual para o usuário, e você não quiser recarregar completamente a página porque você atigiria o servidor novamente e, pelo menos, reiniciaria a aplicação do Nuxt inteira.
