...
Ao contrário do `fetch`, a promessa retornada pelo gatilho `asyncData` é resolvida _durante a transição da rota_. Isto significa que nenhum "placeholder de carregamento" está visível durante as transições do lado do cliente (se bem que a [barra de carregamento](https://v2.nuxt.com/guides/features/loading/) pode ser usada para indicar um estado de carregamento ao usuário). O Nuxt irá ao invés disso esperar que o `asyncData` seja terminado antes de navegar para a próxima página ou exibir a [página de erro](/docs/directory-structure/layouts#error-page).
