...
Usar uma memória para gerenciar o estado é importante para toda grande aplicação. Isto é o porquê do Nuxt implementar o Vuex dentro do seu núcleo.

## Ativar a memória (store)
...
Todo ficheiro `.js` dentro do diretório `store` é transformado em um [módulo com nome reservado](http://vuex.vuejs.org/en/modules.html)(com o `index` sendo a raíz do módulo). O seu valor de `state` sempre será uma `function` para evitar estado *partilhado* indesejado no lado do servidor.
...
O método do módulo também funciona para definições de alto-nível sem a implementação de um subdiretório dentro do diretório store.

Exmplo para estado (state): você cria um ficheiro `store/state.js` e adiciona o seguinte.
...
## Os plugins dentro da memória (store)

Você pode adicionar plugins adicionais à memória (store) ao por eles dentro do ficheiro `store/index.js`:
...
## A ação (action) nuxtServerInit

Se a ação (action) `nuxtServerInit` estiver definida dentro da memória (store) e o modo estiver em `universal`, o Nuxt irá chamar ele com o contexto (apenas a partir do lado do servidor). É útil quando nós temos algum dado no servidor que nós queremos dar diretamente para o lado do cliente.
