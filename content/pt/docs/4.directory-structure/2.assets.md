...
description: O diretório assets contém assets não compilados taís como ficheiros Stylus ou Sass, imagens, ou fontes.
...
O diretório `assets` contém recursos não compilados taís como ficheiros Stylus ou Sass, imagens, ou fontes.
---
...
O Nuxt permite você define os ficheiros/módulos/bibliotecas CSS que você quiser definir globalmente (íncluido dentro de cada página). Dentro do nuxt.config você pode facilmente adicionar seus estilos usando a propriedade CSS.
...
O Nuxt irá automaticamente advinhar o tipo do ficheiro pela sua extensão e usar o carregador do préprocessador adequado para o webpack. Você continuará a precisar instalar os carregador requerido se você precisar eles.
...
O [vue-loader](http://vue-loader.vuejs.org/) processa automaticamente seu estilo e ficheiros de modelos com o `css-loader` e o compilador de modelos do Vue. Neste processo de compilação, todas URLs do recurso taís como `<img src="...">`, `background: url(...)`, e os `@import` do CSS são resolvidos como módulo de dependências.
...
::alert{type="warning"}
O apelodo `~/` não será resolvido corretamente dentro de ficheiros CSS. Você deve ysar `~assets` (**sem uma barra**) dentro das referências `url` do CSS, exemplo `background: url("~assets/banner.svg")`
::
...
O `file-loader` permite você designar onde copiar e colocar o ficheiro de asset, e como nomear ele usando a versão com hash para um cacheamento melhor. Em produção, você tirará proveito de cacheamento de longo período por padrão!
...
O que siginifica que cada ficheiro abaixo de 1kb será embutido como URL de dado na base64. Caso contrário, o imagem/fonte serão copiados para suas pastas correspondentes (dentro do diretório `.nuxt`) com o nome contendo a versão em hash para um cacheamento melhor.
...
Por padrão o diretório fonte (srcDir) e diretório raíz (rootDir) são o mesmo. Você pode usar o apelido `~` para o diretório fonte. Ao invés de escrever caminhos relativos como `../assets/your_image.png` você pode usar `~/assets/your_image.png`.
...
Nós recomendamos usar o `~` como um apelido. `@` continua sendo suportado mas não funcionará em todos casos taís como com imagens de fundo dentro do seu css.

Você pode usar os apelidos `~~` ou  `@@` para o diretório raíz.
