...
description: Dê poderes a sua aplicação Nuxt com o módulo @nuxt/content onde você pode escrever dentro de um diterório content/ e requisitar os seus ficheiros markdown, json, yaml e cs através de uma API parecida com o MongoDB, agindo como um CMS Headless baseado em Git.
category: directory-structure
---
# O diretório content

Dê poderes sua aplicação Nuxt com o módulo `@nuxt/content` onde você pode escrever dentro de um diterório `content/` e requisitar os seus ficheiros markdown, json, yaml e csv através de uma API parecida com o MongoDB, agindo como um **CMS Headless baseado em Git**.
...
Você pode usar componentes Vue diretamente dentro do seus ficheiros markdown. No entanto você precisará usar seus componentes em kebab case e não pode usar tags com auto fechamento.
...
Originalmente o markdown não suporta linhas realçamento de linhas dentro de um bloco de código nem de nomes de ficheiros. O módulo de conteúdo permite isto com sua própria sintáxe personalizada. Linhas numeradas são adicionadas a tag `pre` dentro dos atributos data-line e o nome do ficheiro será convertido para um `span` com uma classe `filename`, assim você pode estilizar ela.
...
### A poderosa API contrutora de consulta (parecida com MongoDB)
...
O módulo de conteúdo funciona com geração de site estático usando o `nuxt generate`. Todas as rotas serão automaticamente gerada graças ao funcionalidade rastejadora do Nuxt.
