...
## A sub-propriedade cssSourceMap
...
O `loaders` tem a mesma estrutura de objeto que o [build.loaders](#a-sub-propriedade-loaders), assim você pode mudar as opções de `loaders` dentro do `extend`.
...
Este exemplo muda pedaços agradáveis de nomes para identificadores númericos:
...
## A sub-propriedade friendlyErrors
...
### A sub-propriedade loaders.file
...
### A sub-propriedade loaders.pugPlain
...
### A sub-propriedade loaders.less
...
### A sub-propriedade plugins do postcss e o módulo @nuxtjs/tailwindcss
...
## A sub-propriedade quiet

> Suprime a maioria do registo de saída da construção 

- Tipo: `Boolean`
- Padrão: Ativado quando uma `CI` ou ambiente de `test` é detectado pelo [std-env](https://github.com/blindmedia/std-env)

## A sub-propriedade splitChunks
...
## A sub-propriedade styleResources
...
Isto é útil quando você precisa injetar algumas váriaveis e mixins dentro das suas páginas sem ter que importar eleas a todo momento.

O Nuxt usa o [style-resources-loader](https://github.com/yenshih/style-resources-loader) para alcançar este comportamento.

Você precisa especificar os padrões/caminos que você quer incluir para os dados pré-processadores: `less`, `sass`, `scss` ou `stylus`
...
## A sub-propriedade templates

> O Nuxt permite você fornecer os seus próprios módelos os quais serão renderizados com base na configuração do Nuxt. Esta funcionalidade é especialmente útil para uso com [módulos](/docs/directory-structure/modules).
...
## A sub-propriedade transpile

- Tipo: `Array<String | RegExp | Function>`
- Padrão: `[]`

Se você quiser transpilar dependências específicas com Babel, você pode adicionar elas dentro do `build.transpile`. Cada item dentro da propriedade `transpile` pode ser um nomde de pacote, uma sequência de caracteres e objeto de expressões regulares correspondem ao nome do ficheiro da dependência.
...
## A sub-propriedade vueLoader

> Nota: Esta configuração tem sido removida desde a versão 2.0 do Nuxt, ao invés desta use o [`build.loaders.vue`](#a-sub-propriedade-loaders).
...
## A sub-propriedade followSymlinks

> Por padrão, o processo de construção não escanea ficheiros dentro de ligações simbólicas (symlinks). Este booleano inclui elas, permitindo assim o uso das ligações simbólicas dentro de pastas tais como a pasta "pages", por exemplo.
