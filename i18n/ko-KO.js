module.exports = {
  ...
  homepage: {
    ...
    welcome: {
      title:
        '직관적인 <br><span class="text-nuxt-lightgreen">Vue</span> 프레임워크<br>',
      description:
    ...
    why: {
      title: '왜 NUXT<span class="text-nuxt-lightgreen">JS</span> 일까요?',
      try_nuxtjs_online: 'NuxtJS Online 시작하기',
      enjoyable: {
        title: 'Enjoyable',
        description:
          '우리는 개발자 경험을 중시합니다. Nuxt.js를 사랑하는 마음으로 이 프레임워크를 개발하고 있으며, 여러분들도 이 프레임워크를 사랑할 수 있으리라 생각합니다💚 매력적인 솔루션, 자세한 에러 메세지, 강력한 디폴트들과 상세한 문서화를 기대해주세요. 질문이나 이슈가 있으시다면, 우리의 커뮤니티가 여러분을 기꺼이 도와드리겠습니다.'
      },
      modular: {
        title: 'Modular',
        description:
    ...
    modes: {
      ...
      ssr: {
        title: '서버 사이드 렌더링',
        description:
          '가장 Nuxt에서 인기있는 모드입니다. "범용(universal)" 혹은 "동형(isomorphic)" 모드라고도 불리는 SSR로, Node.js 서버는 순수 자바스크립트 대신 Vue 컴포넌트에 기반한 HTML을 클라이언트에 전달합니다. SSR로 기존 Vue SPA와 비교했을 때 SEO는 크게 개선될 수 있고, 더 나은 UX 뿐 아니라 더 많은 혜택을 얻을 수 있습니다. <br><br> 여러분이 직접 SSR을 구현하는 것은 매우 귀찮은 작업일 수 있기 때문에, Nuxt.js는 바로 여기에 대한 모든 지원을 제공하며 일반적인 위험요소들을 처리할 것입니다.'
      },
      spa: {
      ...
      ssg: {
        title: '정적 사이트 생성',
        description:
          'JAMStack이라고 불리는 정적 사이트 생성(Static Site Generation)은 지금 아주 핫한 주제 중 하나입니다. 다른 프레임워크로 바꾸어 거기에 익숙해지는 데 시간을 쓰는 대신에, <span style="color: #777">일석이조🐦🐦</span>해보시는건 어떤가요?<br><br> Nuxt.js는 Vue 애플리케이션에 기반한 정적 웹 사이트 생성을 지원합니다. Nuxt는 모든 페이지를 사전 렌더링하고 필요한 HTML을 포함시킬 수 있으므로, "양쪽에 최선"으로 서버 없이 SEO 혜택을 가질 수 있습니다.'
      }
    },
    sponsors: {
      title: '스폰서',
